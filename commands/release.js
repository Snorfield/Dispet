...
async function release(interaction) {
    ...
    if (userObject && userObject.emoji_id) {
        ...
        reply = '✅ Your current emoji pet has been released to be on their own! Try using `/adopt` to get a new one.';
    } else {
        reply = '❌ You don\'t have an emoji pet to release! Try using `/adopt` to get one.';
    }
    return interaction.editReply({ embeds: [simpleEmbed(reply)] });
}
